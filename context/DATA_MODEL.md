...
| `critical_threshold_days` | integer | Varies per medicine (7-15 days). Below this → threshold breach |
| `created_at` | timestamptz | |
...
| `rhu_id` | uuid | FK → `rhu` |
| `medicine_id` | uuid | FK → `medicines` |
| `velocity` | decimal | Units consumed per day (EWMA, α=0.3) |
| `days_remaining` | decimal | Projected days until stockout |
| `last_updated` | timestamptz | |

> **Note:** A unique constraint on `(rhu_id, medicine_id)` is required on `consumption_baselines` to support upsert operations in the velocity engine.
